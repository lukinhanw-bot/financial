...
export const TimelinePoint: React.FC<TimelinePointProps> = ({
  ...
  return (
    <div
      className={`relative flex flex-col items-center min-w-[220px] mx-6 ${
        isAlternate ? 'flex-col-reverse' : 'flex-col'
      }`}
    >
      {/* Balance Card */}
      <motion.div
        className={`
          relative p-5 rounded-2xl shadow-lg cursor-pointer transition-all duration-300 mb-6
          ${isActive 
      ...
      >
        ...
        {hasTransactions && (
          <div className={`flex items-center gap-2 text-sm font-medium ${
            isActive 
              ? netChange >= 0 ? 'text-green-200' : 'text-red-200'
        ...
        <div className={`
          absolute ${isAlternate ? 'top-full' : 'bottom-full'} left-1/2 transform -translate-x-1/2
          w-0 h-0 border-l-[12px] border-r-[12px] border-l-transparent border-r-transparent
          ${isAlternate 
            ? isActive 
              ? 'border-t-[12px] border-t-purple-600' 
              : 'border-t-[12px] border-t-white'
            : isActive 
              ? 'border-b-[12px] border-b-purple-600' 
